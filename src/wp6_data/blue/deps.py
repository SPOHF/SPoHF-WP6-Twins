...
# Project values for manually-uploaded data (e.g. "insects") — the readings
# `project` equals the metadata sensor `source`. Manual data is farm-level,
# not tied to an automated pipeline, so it must stay visible under either
# data-source toggle. Derived from metadata (single source of truth).
MANUAL_PROJECTS: tuple[str, ...] = tuple(sorted({
...
def _project_filter(project: str | None) -> tuple[str, dict[str, Any]]:
    ...
    The data-source toggle selects an *automated* source view; manually-
    uploaded data (``MANUAL_PROJECTS``) is farm-level and always included so
    it appears under either view.

    Returns (sql_fragment, params_dict).
    - project=None  → exclude yookr-direct (already includes manual)
    - project=<str> → that project OR any manual project
    """
    if project is not None:
        if MANUAL_PROJECTS:
            return (
                "(project = %(project)s OR project = ANY(%(manual)s))",
                {"project": project, "manual": list(MANUAL_PROJECTS)},
            )
