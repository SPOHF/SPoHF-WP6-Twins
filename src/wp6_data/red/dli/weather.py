...
def estimate_indoor_par(
    ...
    """Estimate indoor PAR from OpenMeteo weather data.

    Uses trained ML model if available, falls back to physics-based estimation.

    Args:
        solar_radiation: W/m² (shortwave_radiation from OpenMeteo)
        cloud_cover: 0-100%
        transmission: Greenhouse transmission factor (0-1), for physics fallback
        hour: Hour of day (0-23), required for ML model
        day_of_year: Day of year (1-366), required for ML model
        use_model: Whether to try ML model first (default True)

    Returns:
        Estimated indoor PAR in μmol/m²/s
    """
    # Try trained model first
    if use_model and hour is not None and day_of_year is not None:
        from wp6_data.red.dli.model import predict_indoor_par

        predicted = predict_indoor_par(solar_radiation, cloud_cover, hour, day_of_year)
        if predicted is not None:
            return predicted

    # Fallback to physics-based estimation
    if transmission is None:
        transmission = float(os.getenv("WP6_RED_GREENHOUSE_TRANSMISSION", "0.3"))

    par_fraction = 0.45
    wm2_to_umol = 4.57

    outdoor_par = solar_radiation * par_fraction * wm2_to_umol
    indoor_par = outdoor_par * transmission

    return round(indoor_par, 1)


def estimate_daily_natural_dli(
    forecasts: list[DailyForecast],
    transmission: float | None = None,
) -> dict[date, float]:
    """Estimate daily natural DLI from weather forecasts.

    Uses the two-stage ML model if trained, otherwise falls back to physics-based.

    Args:
        forecasts: List of DailyForecast objects
        transmission: Greenhouse transmission factor (used for physics fallback)

    Returns:
        Dict mapping date to estimated indoor DLI (mol/m²/day)
    """
    ...
    for forecast in forecasts:
        ...
        if model.is_trained() and daily_radiation_sum > 0:
            ...
            dli = model.predict_dli(daily_radiation_sum)
        else:
            # Fallback to physics-based estimation
            total_par_seconds = 0.0
            for h in forecast.hourly:
                indoor_par = estimate_indoor_par(
                    h.solar_radiation,
                    h.cloud_cover,
                    transmission,
                    use_model=False,  # Force physics fallback
                )
                # Each hour = 3600 seconds
                total_par_seconds += indoor_par * 3600

            # Convert μmol/m² to mol/m²
            dli = total_par_seconds / 1_000_000

        results[forecast.date] = round(dli, 2)
