...
**Growth section**:
A named canopy zone the plant is divided into for the prescriptive view, mapped one-to-one onto a **Height** in fixed top-to-bottom order, identical for every wire: H1 "Kop" (just above the canopy top), H2 "Bloeiende tros", H3 "Vruchtzetting", H4 "Afrijping", H5 "Substraat/wortelzone" (root zone). The label and ordering are a horticultural assumption declared in config, not a measured distance. H1 sits *above* the plant, so it reads incoming light before the canopy attenuates it.
